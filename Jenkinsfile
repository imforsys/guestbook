...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="imforsys/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/imforsys/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@52.79.61.61 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no root@52.79.61.61 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
                                        -e MYSQL_IP=13.124.32.117 \
                                        -e MYSQL_PORT=3306 \
